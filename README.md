...
이 글은 근무 중 작성한 쿼리 퍼포먼스 튜닝 사례집을 각색·복원한 노트다. 개념 설명의 관점에서 추가·정제된 표현이 있을 수 있다.

금융권, 대기업 SI/SM, 10년 이상 된 외주 프로젝트에서는 1000 라인 이상의 쿼리가 자주 보인다. 고통받는 사람은 많지만 노하우는 잘 공유되지 않는 편이다.  
아래의 상황에서 SQL 퍼포먼스 개선을 해결하는 방법을 공유하려 한다. 
...
본 글의 "temp 테이블" 은 옵티마이저가 쿼리 실행 중 자동 생성하는 internal temporary table 을 가리킨다. 본문은 MySQL 8.0 기준이며, Oracle 구버전은 필요할 때 대비 형식으로 언급한다.

"애초에 그 상황을 바꾸어야 하지 않나" 는 의문은 기술적으로 옳지만, 외주 3~4개를 낀 구조 + 인력 역량 저점 미보장 + **업체 간 배상 계약**이 엮인 환경에서는 구조 변경 자체가 어렵다. 이 글은 **근원 해결이 어려운 전제** 에서 출발한다.

글에 명시된 내용은 시뮬레이션할 수 있는 환경과 결과를 [tuning_test/](tuning_test/) 에 공유한다.  
아래 3장 해결 패턴에 명시된 숫자는 여기의 측정값을 기본으로 한다.
...
1,000줄 레거시 쿼리 앞에서 "무엇부터 봐야 하는가" 의 답은 단순하다. **EXPLAIN 의 `select_type` 에서 DERIVED 행**, `**Extra` 컬럼의 Using temporary / Using filesort** — 이 세 지점만 훑어도 어디서 merge 가 끊겼는지, 어떤 중간 결과가 temp 로 앉았는지 식별할 수 있다.

식별이 되면 판단도 단순해진다. 인덱스·쿼리 재작성이 가능한가? 가능하면 근원을 제거. 어려우면 패턴 A 로 중간 결과를 줄이거나, 패턴 B 로 한 쿼리를 두 단계로 쪼갠다. Oracle 이면 `MATERIALIZE` / `INLINE` hint 가 수동 제어 카드다. 결과가 같은지는 4장의 3단계 검증으로 닫는다.

레거시의 거대함에 압도되기보다, **옵티마이저가 그 블록에서 왜 merge 를 포기했는가** 를 한 블록씩 묻는다. 접근은 이 정도로 단순해진다.
